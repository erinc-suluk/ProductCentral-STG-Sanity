...
public class UMSPage extends HelperFunctions {
	...
	public void setDigitalMIM(ExtentTest test) throws Exception {
		...
		wait1.until(ExpectedConditions.visibilityOf(defaultYes));
		Assert.assertTrue(defaultYes.isDisplayed());
		test.info("Verified yes as default selected from MIM Dropdown");
		HelperFunctions.staticWait(2);
		test.info("Click on cancel button and wait for search product visibility");
		cancelButton2.click();
		wait1.until(ExpectedConditions.visibilityOf(searchCompany));
		test.info("Click on search product field and send Customer Link as key product");
		searchCompany.click();
		String auto="Customer Link";
		searchCompany.sendKeys(auto);
		HelperFunctions.staticWait(2);
		test.info("Click on search button and wait for first product result");
		searchButton.click();
		wait1.until(ExpectedConditions.visibilityOf(firstResult));
		firstResult.click();
		test.info("Wait for edit product button's visibility then click on it");
		wait1.until(ExpectedConditions.visibilityOf(editProduct));
		editProduct.click();
		wait1.until(ExpectedConditions.visibilityOf(defaultYes));
		Assert.assertTrue(defaultYes.isDisplayed());
		test.info("Verified yes is selected as default this also verified the customer link has system indicator on my product page");
		HelperFunctions.staticWait(2);
		test.info("Click on MIM Dropdown");
		MIMdropdown.click();
		WebDriverWait wait3 = new WebDriverWait(Driver.getDriver(), 30);
		wait3.until(ExpectedConditions.visibilityOf(noOption));
		HelperFunctions.staticWait(2);
		Assert.assertTrue(noOption.isDisplayed());
		test.info("Verified there is also no option on MIM dropdown");
		HelperFunctions.staticWait(5);
	}
